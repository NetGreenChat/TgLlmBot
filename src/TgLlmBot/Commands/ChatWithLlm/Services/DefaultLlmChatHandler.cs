...
public partial class DefaultLlmChatHandler : ILlmChatHandler
{
    ...
    public static string BuildBasePrompt(string botName, string? formattedDate = null)
    {
        return $"""
                Ты - полезный чат-бот в групповом чате, тебя зовут {botName}.
    ...
    private async Task<ChatMessage> BuildSystemPromptAsync(ChatWithLlmCommand command, CancellationToken cancellationToken)
    {
        ...
        var basePrompt = BuildBasePrompt(_options.BotName, formattedDate);
